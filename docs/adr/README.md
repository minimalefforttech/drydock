...
| [0019](0019-mcp-registry-and-scoped-memory.md) | MCP registry and scoped memory | Accepted | MCP servers defined once, toggled per workspace/task/chat (tri-state cascade, sensitive opt-in, env never rendered); memory scoped global/workspace/task with glob-rule tags detected at mount time and briefed in provenance groups; agent memory proposals editable at the approval gate; a context-debug doc shows the composed briefing with sources. Amended: the cascade generalizes to product configuration (preferences rung). |
| [0020](0020-repo-providers-and-managed-project-roots.md) | Repo providers and managed project roots | Accepted | GitHub/GitLab picks clone host-side into a managed root and register as ordinary catalog projects with origin provenance. Tokens stay in keychain/SecretStorage and never reach a sandbox; the sandbox never fetches. |
